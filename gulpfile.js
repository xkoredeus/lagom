...
gulp.task('contacts', function () {
    return (
        gulp.src(['src/assets/js/contacts.js'])
            .pipe(gulp.dest('dist/assets/js/'))
            .pipe(browserSync.stream())
    );
});
...
gulp.task('about', function () {
    return (
        gulp.src(['src/assets/js/about.js'])
            .pipe(gulp.dest('dist/assets/js/'))
            .pipe(browserSync.stream())
    );
});

gulp.task('coach', function () {
    return (
        gulp.src(['src/assets/js/couch-page.js'])
            .pipe(gulp.dest('dist/assets/js/'))
            .pipe(browserSync.stream())
    );
});
...
gulp.task('default', ['clean:dist', 'font', 'scripts', 'contacts', 'about', 'coach', 'images', 'compile-html', 'resetPages', 'watch']);

// Creates production ready assets in dist folder
gulp.task('build', function () {
    console.log('Building production ready assets');
    runSequence('clean:dist', 'sass', ['scripts', 'contacts', 'about', 'coach', 'images', 'font', 'compile-html'])
});
